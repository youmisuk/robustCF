...
# twolevel.normalU - data generating function for Design 1
# twolevel.uniformU - data generating function for Designs 2 and 3
# twolevel.binary - data generating function for Design 4
# twolevel.experror - - data generating function for Design 5
...
twolevel.normalU <- function(ids=1, Smpl.size = "150.30", s.val=0.3, o.val=2, m.val=2) {
  ...
  W1 <- runif(J, -1, 1)
  W2 <- rnorm(J, 0, 1) # W2 is an unmeasured cluster-level confounder U_j that follows a normal distribution.
...
twolevel.binary <- function(ids=1, Smpl.size = "150.30", s.val=0.3, o.val=0.3, m.val=0.3) {
  ...
  pop$psY0 <- 1 / (1 + exp(-pop$lpsY0)) # likelihood of being 1 for the control    
  pop$psY1 <- 1 / (1 + exp(-pop$lpsY1)) # likelihood of being 1 for the treated    
